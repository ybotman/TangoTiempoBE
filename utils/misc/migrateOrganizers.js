...
async function migrateOrganizers() {
  ...
  try {
    ...
    const division = region.divisions.find(
      (div) => div.divisionName === "New England",
    );
    if (!division)
      throw new Error('Division "New England" not found in region "Northeast"');

    defaultDivision = division._id;

    const city = division.majorCities.find(
      (city) => city.cityName === "Boston",
    );
    if (!city)
      throw new Error('City "Boston" not found in division "New England"');
  ...
  const apiEndpoint =
    "https://bostontangocalendar.com/wp-json/tribe/events/v1/organizers?per_page=200&page=1";
  let organizersData;
  try {
  ...
  for (const organizer of organizersData) {
    try {
      logger.info(
        `Processing organizer "${organizer.organizer}" with slug "${organizer.slug}"`,
      );
      ...
      if (organizer.image && organizer.image.url) {
        transformedOrganizer.images.push({
      ...
      if (result.upsertedCount > 0) {
        ...
        logger.info(
          `Organizer "${transformedOrganizer.name}" already up-to-date.`,
        );
      }
      ...
      logger.error(
        `Error processing organizer "${organizer.organizer || "unknown"}": ${error.message}`,
      );
  ...
  logger.info(
    `Total organizers processed: ${processedCount}/${organizersData.length}`,
  );
...
function resetImportBTCOrganizers() {
  try {
    ...
    const updatedEnv = envFileContent
      .split("\n")
      .map((line) =>
        line.startsWith("ImportBTCOrganizers=")
          ? "ImportBTCOrganizers=false"
          : line,
      )
      .join("\n");
    ...
    logger.info(
      "ImportBTCOrganizers has been set to false in current process runtime.",
    );
  } catch (error) {
