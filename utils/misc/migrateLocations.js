...
async function migrateLocations() {
  ...
  const apiBaseUrl =
    "https://bostontangocalendar.com/wp-json/tribe/events/v1/venues?per_page=100";
  let venuesData = [];
  let page = 1;
  let morePages = true;
  ...
  try {
    while (morePages) {
      ...
      const currentPageData = response.data.venues;
      if (!currentPageData || currentPageData.length === 0) {
        morePages = false;
      } else {
        venuesData.push(...currentPageData);
        page += 1;
        if (!response.data.next_rest_url) {
          morePages = false;
        }
      }
    }
  ...
  for (const venue of venuesData) {
    try {
      ...
      const result = await Locations.updateOne(
        {
          name: transformedLocation.name,
          address_1: transformedLocation.address_1,
        },
      ...
      if (result.upsertedCount > 0) {
        ...
        logger.info(
          `Location "${transformedLocation.name}" already up-to-date.`,
        );
      }
    } catch (error) {
      logger.error(
        `Error processing location "${venue.venue}": ${error.message}`,
      );
    }
  }
...
function resetImportBTCLocations() {
  try {
    ...
    const updatedEnv = envFileContent
      .split("\n")
      .map((line) =>
        line.startsWith("ImportBTCLocations=")
          ? "ImportBTCLocations=false"
          : line,
      )
      .join("\n");
    ...
    logger.info(
      "ImportBTCLocations has been set to false in current process runtime.",
    );
  } catch (error) {
