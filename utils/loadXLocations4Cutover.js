        ...
        parser.parseString(xmlData, async (err, result) => {
            ...
            for (const item of items) {
                try {
                    ...
                    if (!address_1) throw new Error('Address is missing');
                    if (!city) throw new Error('City is missing');
                    if (!state) throw new Error('State is missing');
                    if (!zip) throw new Error('ZIP code is missing');
                    ...
                    logger.info(`Location ${name} loaded successfully`);
                    successfulLocations.push({ name });

                } catch (error) {
                    logger.error(`Error loading location: ${item.title} - ${error.message}`);
            ...
            logger.info('Location loading complete');
